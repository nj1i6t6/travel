// 【最終修正方案】
// 1. 使用新的 @google/genai ESM 函式庫
// 2. 將 API 金鑰直接寫入
// 3. 使用 gemini-2.5-pro 模型
// 4. 啟用 googleSearch 連網工具

import { GoogleGenerativeAI } from "https://cdn.jsdelivr.net/npm/@google/genai/+esm";
import { openDB } from "https://cdn.jsdelivr.net/npm/idb@7/+esm";

// --- 靜態配置 ---
const API_KEY = "AIzaSyC2l7mrzCMYcZ33pAOldgVbBiCxGvBuizc";
const CHAT_MODEL_NAME = "gemini-2.5-pro";
const JSON_MODEL_NAME = "gemini-2.5-pro"; // 也可以使用 flash 來節省成本，但 pro 更強大
...
// 初始化 Gemini AI 客戶端和模型
function initializeGemini() {
    try {
        genAI = new GoogleGenerativeAI(API_KEY);

        // 初始化用於一般聊天的模型，並啟用連網工具
        chatModel = genAI.getGenerativeModel({
            model: CHAT_MODEL_NAME,
            tools: [{ googleSearch: {} }], // 啟用 Google 搜尋連網功能
        });
        ...
        console.log(`AI 初始化成功！聊天模型: ${CHAT_MODEL_NAME}, JSON 模型: ${JSON_MODEL_NAME}`);
        return true;
    } catch (error) {
...
async function generateTripJson(userPromptContext) {
    ...
    const prompt = `
    請根據以下使用者需求和對話內容，為我生成一個完整的旅遊行程計畫的 JSON 資料。
    JSON 的根物件應包含一個 "trip" 物件和一個 "dailyPlans" 陣列。
    "trip" 物件應包含: "name" (行程名稱, string), "country" (國家, string), "startDate" (開始日期, 'YYYY-MM-DD' string), "endDate" (結束日期, 'YYYY-MM-DD' string)。
    "dailyPlans" 陣列包含多個每日計畫物件。每個每日計畫物件應包含: "date" ('YYYY-MM-DD' string), "notes" (當日總結, string, 可選), 和一個 "items" 陣列。
    每個 "items" 行程項目物件應包含: "name" (名稱, string), "type" (類型, string，可為 '景點', '交通', '住宿', '餐飲', '其他'), "cost" (預估花費, number, 可選), "timeEstimate" (預估時間(小時), number, 可選), "notes" (備註, string，可包含網址或圖片URL, 可選)。
    所有可選欄位若無確切資訊可留空或省略。
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const showTripsBtn = document.getElementById('show-trips-btn');
    const tripListModal = document.getElementById('trip-list-modal');
    const tripListContent = document.getElementById('trip-list-content');
    const closeTripListBtn = tripListModal.querySelector('.close-btn');
